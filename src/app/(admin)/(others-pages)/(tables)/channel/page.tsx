...
import ChannelTable, { ColumnSelector } from "@/components/tables/ChannelTable";
import React, { useState, useEffect } from "react";
import Button from "@/components/ui/button/Button";
import Link from "next/link";
import { PlusIcon, BuildingOfficeIcon, HomeIcon } from "@heroicons/react/24/outline";
import { ChannelType, channelData } from "@/components/form/sample-data/channelData";
...
export default function ChannelPage() {
  ...
  const [visibleColumns, setVisibleColumns] = useState<(keyof ChannelType)[]>([
    'channel_type_id', 'channel_type_name', 'channel_type_description', 'created_date', 'updated_date', 'status'
  ]);

  // Channel statistics
  const totalChannels = channelData.length;
  const activeChannels = channelData.filter(ch => ch.is_active).length;
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="bg-white dark:bg-dark-800 rounded-xl border border-gray-200 dark:border-white/[0.05] p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900 dark:text-white">{activeChannels}</p>
            </div>
      ...
      <div className="space-y-6">
        <ComponentCard title="Channel Management">
