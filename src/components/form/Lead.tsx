...
import { ILead, leadsData } from "./sample-data/leadsData";
// import api from "@/lib/api"; // Ready for API integration

// Re-export ILead interface for other components
export type { ILead };

// Sample lead data - replace with actual API call
const sampleLeads: ILead[] = leadsData;
...
export default function Lead({ value, onChange, error: validationError }: LeadProps) {
  ...
  const fetchLeads = useCallback(async (page: number = 1, search: string = "") => {
      ...
      >
        <span className="truncate text-left">
          {selectedLead ? (
            ...
            <span className="text-gray-400 dark:text-white/30">Click to select lead</span>
          )}
        </span>
        <svg
        ...
        {filteredLeads.length > 0 && !loading && !error && (
          <div className="flex items-center justify-between mt-6 px-6 py-4">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">
              Showing {Math.min((currentPage - 1) * itemsPerPage + 1, filteredLeads.length)} to {Math.min(currentPage * itemsPerPage, filteredLeads.length)} of {filteredLeads.length} results
            </p>
