 ...
 *    - fetchStaff() function ready for endpoint connection
 ...
 *    - GET /api/staff?page={page}&limit={limit}&search={search}
 *    - Response: { staff: IStaff[], totalPages: number, currentPage: number, totalItems: number }
 * 
 * 4. FEATURES:
 ...
 * 
 * TO ACTIVATE API: Uncomment the API call in fetchStaff() function and provide the endpoint URL
 */
...
import { staffData } from "./sample-data";
...
interface StaffProps {
  value?: IStaff;
  onChange?: (staff: IStaff) => void;
  error?: string;
}

export default function Staff({ value, onChange, error: validationError }: StaffProps) {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [selectedStaff, setSelectedStaff] = useState<IStaff | null>(value || null);
  ...
  const fetchStaff = useCallback(async (search: string = "") => {
    ...
    try {
      ...
      // 
      // Expected API Endpoint: GET /api/staff
      // Query Parameters:
      ...
      // const response = await api.get(`/api/staff?page=${page}&limit=${itemsPerPage}&search=${encodeURIComponent(search)}`);
      ...
      if (search) {
        filteredData = staffData.filter((staffMember) =>
          staffMember.StaffFullName.toLowerCase().includes(search.toLowerCase()) ||
          staffMember.StaffID.toLowerCase().includes(search.toLowerCase()) ||
          staffMember.Phone?.toLowerCase().includes(search.toLowerCase()) ||
          staffMember.Department?.toLowerCase().includes(search.toLowerCase())
        );
      }
  ...
  // Get staff for current page (client-side pagination for now)
  ...
  }, [searchTerm, isModalOpen, fetchStaff]);
  ...
  }, [isModalOpen, currentPage, fetchStaff, searchTerm]);
  ...
  // Get current staff to display (use API data when available, fallback to filtered sample data)  
  const filteredStaff = staff.length > 0 ? staff : staffData.filter((staffMember) =>
    staffMember.StaffFullName.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const displayStaff = getCurrentPageStaff();
    ...
    <div>
      ...
      >
        <span className="truncate text-left">
          {selectedStaff ? (
            <span className="text-gray-800 dark:text-gray-200">{`${selectedStaff.StaffID} - ${selectedStaff.StaffFullName}`}</span>
          ) : (
            <span className="text-gray-400 dark:text-white/30">Click to select staff member</span>
          )}
        </span>
        <svg
      ...
      <Modal 
        ...
        className="max-w-[1000px] p-4 lg:p-11"
      >
        <div className="px-2 lg:pr-14">
          ...
          <p className="mb-6 text-sm text-gray-500 dark:text-gray-400 lg:mb-7">
            Choose a staff member to associate with this call log.
          </p>
        </div>
                    ...
                    {displayStaff.map((staff) => (
                      ...
                      >
                        <TableCell className="px-5 py-2 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                          <div onClick={() => handleStaffSelect(staff)} className="w-full h-full">
                            {staff.StaffID}
                          </div>
                        </TableCell>
        ...
        {filteredStaff.length > 0 && !loading && !error && (
          <div className="flex items-center justify-between mt-6 px-6 py-4">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">
              Showing {Math.min((currentPage - 1) * itemsPerPage + 1, filteredStaff.length)} to {Math.min(currentPage * itemsPerPage, filteredStaff.length)} of {filteredStaff.length} results
            </p>
